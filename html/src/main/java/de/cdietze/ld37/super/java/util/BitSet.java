...
/**
 * This implementation uses bit groups of size 32 to keep track of when bits are set to true or false.  This
 * implementation also uses the sparse nature of JavaScript arrays to speed up cases when very few bits are set in a
 * large bit set.
 * <p/>
 * Since there is no speed advantage to pre-allocating array sizes in JavaScript the underlying array's length is shrunk
 * to Sun's "logical length" whenever length() is called.  This length is the index of the highest true bit, plus one,
 * or zero if there are aren't any.  This may cause the size() method to return a different size than in a true Java
 * VM.
 */
public class BitSet {
  ...
  /**
   * This hash is different than the one described in Sun's documentation.  The described hash uses 64 bit integers and
   * that's not practical in JavaScript.
   */
  @Override
  public int hashCode() {
    // FNV constants
